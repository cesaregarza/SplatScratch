...
VERSION_URL = (
    "https://github.com/Leanny/splat3/tree/main/data/mush"
)
LANG_URL = "Leanny/splat3/main/data/language/"

WeaponsMap: TypeAlias = dict[str, dict[str, str | float]]


@cache
def enumerate_versions(return_metadata: bool = False) -> list[str] | ResultSet:
    """Get a list of all the versions of the datamine.
    ...
    versions = [
        x for x in 
        data["payload"]["tree"]["items"]
    ]
    if return_metadata:
        return versions
    return [x["name"] for x in versions]
...
def get_version_url(version: str | None = None) -> str:
    ...
    """
    versions = enumerate_versions(return_metadata=True)
    ...
    if version is None:
        ...
        selected_version = [v for v in versions if v["name"] <= version][-1]
    out_url: str = RAW_URL + "Leanny/splat3/main/" + selected_version["path"] + "/"
    return out_url
